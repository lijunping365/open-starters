 ...
 */
@Slf4j
public class DefaultCacheListenerErrorHandler implements CacheListenerErrorHandler {

    @Override
    public void onListenerError(CacheExecuteException exception, CacheMessage message) {
        final String cacheName = message.getCacheName();
        CacheCommand command = message.getCommand();
        log.error("监听缓存消息执行异常 = {}，cacheName = {}, 异常原因 = {}", command.name(), cacheName, message);
    }
}
